...
/*	TODO:	Перегрузить операторы: -, -=, prefix --, ssuffix --, >>  */
...
public:
	...
	// Methods
	double distance(Point other) //Метод вычисляет расстояние до указанной точки
	{
...
double distance(Point A, Point B)//Функция вычисляет расстояние до указанной точки
{
...
int main()
{
	...
	A.operator+=(B); //Явный вызов оператора. Он вызывается как обычный метод для обьекта А
	A.print();
#endif
